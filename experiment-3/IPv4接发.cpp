...
struct Ipv4
{
  ...
  Ipv4(unsigned int len,unsigned int srcAddr,unsigned int dstAddr,
    byte _protocol,byte ttl) {//有参构造函数，封装IPV4报文
    memset(this,0,sizeof(Ipv4));
    version_ihl = 0x45;
    total_length = htons(len+20);//20字节头部
    time_to_live = ttl;
    protocol = _protocol;
    ...
    int sum = 0;
    for(int i = 0; i < 10; i++) {//cheksum，求校验和
        if(i != 5) {
...
int stud_ip_recv(char *pBuffer,unsigned short length)
{
  ...
  *ipv4 = *(Ipv4*)pBuffer;
  ...
  int ihl = 0xf & ipv4->version_ihl;//首部长度校验
  if(ihl < 5) {
  ...
  if(destination_address != getIpv4Address() && destination_address != 0xffffffff) {
    ip_DiscardPkt(pBuffer,STUD_IP_TEST_DESTINATION_ERROR);//目的地址不是本机或者广播地址
    return 1;
  }
